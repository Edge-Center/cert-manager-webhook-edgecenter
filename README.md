...
```bash
helm install -n cert-manager edgecenter-webhook --set pod.securePort=8443 ./deploy/helm
```

- To uninstall the webhook:
```bash
$ helm delete edgecenter-webhook -n cert-manager
```
...
In order to communicate with EdgeCenter DNS provider, we will create a Kubernetes Secret
to store the EdgeCenter `API` and `EdgeCenter Secret`.
...
- Create a `ClusterIssuer`resource to specify the address of the ACME staging or production server to access.
  Add the DNS01 Solver Config that this webhook will use to communicate with the API of the EdgeCenter Server in order to create
...
To test one of your registered domains on EdgeCenter, create a secret.yml file using as [example] file(./testdata/ec/apikey.yaml)
Replace the token value with your Edge-Center API permanent token which corresponds to your `345...`
...
```
**IMPORTANT**: As EdgeCenter server could be very slow to reply, it could be needed to increase the TTL defined within the `config.json` file. The test could also fail
