...
func main() {
	...
	cmd.RunWebhookServer(groupName,
		&edgecenterDNSProviderSolver{},
	)
}

// edgecenterDNSProviderSolver implements the provider-specific logic needed to
// 'present' an ACME challenge TXT record for your own DNS provider.
// To do so, it must implement the `github.com/cert-manager/cert-manager/pkg/acme/webhook.Solver`
// interface.
type edgecenterDNSProviderSolver struct {
	client             *kubernetes.Clientset
	ttl                int
	propagationTimeout int
}

// edgecenterDNSProviderConfig is a structure that is used to decode into when
...
type edgecenterDNSProviderConfig struct {
...
func (c *edgecenterDNSProviderSolver) Name() string {
	return providerName
}
...
func (c *edgecenterDNSProviderSolver) Present(ch *v1alpha1.ChallengeRequest) error {
...
// concurrently.
func (c *edgecenterDNSProviderSolver) CleanUp(ch *v1alpha1.ChallengeRequest) error {
	sdk, err := c.initSDK(ch)
	if err != nil {
		return fmt.Errorf("init sdk: %w", err)
	}
...
func (c *edgecenterDNSProviderSolver) Initialize(kubeClientConfig *rest.Config, _ <-chan struct{}) error {
...
func (c *edgecenterDNSProviderSolver) upsertTxtRecord(ctx context.Context, sdk *dnssdk.Client, ch *v1alpha1.ChallengeRequest) error {
	fqdn := strings.Trim(ch.ResolvedFQDN, ".")
	zone, err := c.detectZone(ctx, fqdn, sdk)
	if err != nil {
		return fmt.Errorf("detect zone: %w", err)
...
func (c *edgecenterDNSProviderSolver) initSDK(ch *v1alpha1.ChallengeRequest) (*dnssdk.Client, error) {
	...
	if apiFullUrl == "" {
		apiFullUrl = "https://api.edgecenter.ru/dns"
	}
...
func (c *edgecenterDNSProviderSolver) extractApiTokenFromSecret(
	cfg edgecenterDNSProviderConfig, ch *v1alpha1.ChallengeRequest) (string, error) {
...
func (c *edgecenterDNSProviderSolver) detectZone(ctx context.Context, fqdn string, sdk *dnssdk.Client) (string, error) {
...
func loadConfig(cfgJSON *extapi.JSON) (edgecenterDNSProviderConfig, error) {
	cfg := edgecenterDNSProviderConfig{}
